    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        Parameters:
            ...
            code_type: (:obj:`str`, `optional`):
                The targeted type of codes for generation, e.g., icd diagnosis codes, operation codes,
                lab test codes, or drug codes.
            ...
            model_kwargs:
                ...
                in this case it has ['use_cache','attention_mask','encoder_outputs']
        Return:
        ...
        while True:
            # continue to generate untill it encounters eos mark as early stopping
            if synced_gpus:
            ...
            # set the forward token type id here
            outputs = self(
                **model_inputs,
                return_dict=True,
            ...
            next_token_scores = logits_processor(logits_ids, next_token_scores)
            ...
            # decode input_ids from the code vocab (1071) to the general vocab (50000+) before input for next step inference
            new_beam_next_tokens = beam_next_tokens.cpu().numpy()+self.ehr_tokenizer_dict.label_offset
            new_beam_next_tokens_str = self.ehr_tokenizer_dict.tokenizer_dict[code_type].decode(new_beam_next_tokens)
            new_beam_next_tokens = new_beam_next_tokens_str.split()
            new_beam_next_tokens = np.array(new_beam_next_tokens).astype(int)
            new_beam_next_tokens = torch.tensor(new_beam_next_tokens).to(beam_next_tokens.device)

            input_ids = torch.cat([input_ids[beam_idx, :], new_beam_next_tokens.unsqueeze(-1)], dim=-1)
            logits_ids = torch.cat([logits_ids[beam_idx, :], beam_next_tokens.unsqueeze(-1)], dim=-1)
    ...
    def beam_sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # debug: because of no_repeat_ngram_size = 1, probs can get nan
            # TODO
        ...
        # remove the eos token id at last
        # sequence_outputs['sequences'] = sequence_outputs['sequences'][:,:-1]
